let output = [], delivery1 = "Alexis", delivery2 = "Maia", delivery3 = "Barbi", retiro = "retirado", pagado = "pagado";
const comision = 60;
...
const recibirData = async () => {
	let petiGETPedidos = await fetch("http://localhost:3001/pedidos")
				...
				if (re.id == item.children[0].textContent) {
					...
					item.children[5].children[1].addEventListener("click", () => {
						...
						const petitPUT = async () => {
							let da = await fetch(`http://localhost:3001/pagado/${item.children[0].textContent}`, {
								method: "PUT",
								body: '{"pagado":0}',
					...
					if (item.children[5].innerHTML != retiro) {
						if (item.children[5].innerHTML != pagado) {
							...
							item.children[5].children[0].children[5].addEventListener("click", () => {
								fetch(`http://localhost:3001/pagado/${item.children[0].textContent}`, {
									method: "PUT",
									body: '{"pagado":1}',
									headers: { "content-type": "application/json" }
								})
							...
							item.children[5].children[0].children[4].addEventListener("click", () => {
								...
								const statuPUT = async (id, name) => {
									let da = await fetch(`http://localhost:3001/statu/${item.children[0].textContent}`, {
										method: "PUT",
							...
							const borrarUnPedido = async () => {
								let pedidoDELETE = await fetch(`http://localhost:3001/pedidos/${item.children[7].textContent}`, {
									method: "DELETE"
								})
								window.location.reload();
							}
							...
							item.children[5].children[0].children[10].addEventListener("click", () => {
								if (item.children[3].style.color == "") {
									item.children[3].innerHTML += ` <b>(PJ)</b>`
									$(item.children[3]).css("color", "#16ea31")
									fetch(`http://localhost:3001/pj/${item.children[0].textContent}`, {
										method: "PUT",
										body: '{"pj":1}',
										headers: { "content-type": "application/json" }
									})
