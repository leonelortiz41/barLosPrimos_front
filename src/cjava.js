...
const recibirData = async (milanesasdb, lomitosdb, hamburguesasdb, pizzasdb, papasdb, platosdb, bebidasdb) => {
	...
	if (milanesasLS == null) {
		milanesasdb = await fetch("http://localhost:3001/milanesas"); milanesasdb = await milanesasdb.json();
		localStorage.setItem("milanesasLS", JSON.stringify(milanesasdb));
	}
	else milanesasdb = JSON.parse(milanesasLS);

	if (lomitosLS == null) {
		lomitosdb = await fetch("http://localhost:3001/lomitos"); lomitosdb = await lomitosdb.json();
	...
	if (hamburguesasLS == null) {
		hamburguesasdb = await fetch("http://localhost:3001/hamburguesas"); hamburguesasdb = await hamburguesasdb.json();
	...
	if (pizzasLS == null) {
		pizzasdb = await fetch("http://localhost:3001/pizzas"); pizzasdb = await pizzasdb.json();
		localStorage.setItem("pizzasLS", JSON.stringify(pizzasdb));
	}
	else pizzasdb = JSON.parse(pizzasLS);

	if (papasLS == null) {
		papasdb = await fetch("http://localhost:3001/papas"); papasdb = await papasdb.json();
		localStorage.setItem("papasLS", JSON.stringify(papasdb));
	}
	else papasdb = JSON.parse(papasLS);

	if (platosLS == null) {
		platosdb = await fetch("http://localhost:3001/platos"); platosdb = await platosdb.json();
	...
	if (bebidasLS == null) {
		bebidasdb = await fetch("http://localhost:3001/bebidas"); bebidasdb = await bebidasdb.json();
	...
	$("#JuniorDobleComun").click((e) => {
		selectComida("papas", hamburguesasdb[5].nombre, hamburguesasdb[5].precio, e.target.classList[1])
	}).addClass(colorHamPapas)

	$("#JuniorDobleEspecial").click((e) => {
		selectComida("papas", hamburguesasdb[6].nombre, hamburguesasdb[6].precio, e.target.classList[1])
	}).addClass(colorHamPapas)
	...
	const enviarData = async () => {
		...
		let petiPOST = await fetch("http://localhost:3001/pedidos", {
			method: "POST",
			body: JSON.stringify(pedidoFinal),
			headers: { "content-type": "application/json" }
		})
		let dataPOST = await petiPOST.json();
		let petiGET = await fetch("http://localhost:3001/pedidos")
				...
				if (pedidoFinal.pedido.length != 0) {
					...
					if (edit != null) {
						const peticionPut = async () => {
							...
							const resPut = await fetch(`http://localhost:3001/pedidos/${JSON.parse(edit).id}`, {
								method: "PUT",
								body: JSON.stringify(pedidoFinal),
								headers: { "content-type": "application/json" }
							})
