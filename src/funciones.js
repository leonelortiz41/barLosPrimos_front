...
const darOk = (ejecucion, delivery, hidden_delivery, viajes, repartoCliente, hd, importe, ar1, ar2, r) => {
	if (ejecucion == true) {
		const statuPUT = async (id, name) => {
			let da = await fetch(`http://localhost:3001/statu/${id}`, {
				method: "PUT",
...
const cerrarSesion = async () => {
	...
	if (option == true) {
		...
		let pedidosDELETE = await fetch('http://localhost:3001/pedidos', {
			method: "DELETE"
		})
...
const ticketCosina = (numero, clienteP, detalle) => {
	...
	cliente.innerHTML = clienteP.toLowerCase();
...
const ticketDelivery = (hidden_delivery, r) => {
	...
	let table = w.document.createElement('div');
	let fila = [], celdaCliente = [], celdaImporte = [],celdaBebida=[],hr=[],detalle=[],tittle;
	tittle = w.document.createElement('p');
	div.appendChild(tittle);
	tittle.innerHTML="delivery: "+hidden_delivery[0].children[5].textContent
	div.appendChild(table);
	for (var i = 0; i <= hidden_delivery.length - 1; i++) {

		fila[i] = w.document.createElement('div');
		table.appendChild(fila[i]);
		detalle[i] = w.document.createElement('div');
		fila[i].appendChild(detalle[i]);
		celdaCliente[i] = w.document.createElement('p');
		detalle[i].appendChild(celdaCliente[i]);
		celdaImporte[i] = w.document.createElement('p');
		detalle[i].appendChild(celdaImporte[i]);
		celdaBebida[i] = w.document.createElement('p');
		fila[i].appendChild(celdaBebida[i]);
		...
		for (let j = 0; j < r.length; j++) {
			if (r[j].id == hidden_delivery[i].children[0].textContent) {
				...
				JSON.parse(`[${r[j].detalle}]`).forEach((e)=>{
					if(e.hidden_clase=="bebida"){
						if(init==true){
							celdaBebida[i].innerHTML+="va con: "+`<br/>`;
							init=false;	
						}
						celdaBebida[i].innerHTML+=e.hidden_cant+" "+e.hidden_nombre+`<br/>`
					}
				})
			}
		}
